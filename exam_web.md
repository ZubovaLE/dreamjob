# Контрольные вопросы. Web:

## Оглавление

## [Git:](#Git)

1. [Что такое индекс?](#1-Что-такое-индекс)
2. [Как добавить изменения в индекс?](#Как-добавить-изменения-в-индекс) (git add .)
3. [Как сделать фиксацию изменений?](#Как-сделать-фиксацию-изменений) (git commit -m "text")
4. [Как сделать синхронизацию изменений с удаленным репозиторием?](#Как-сделать-синхронизацию-изменений-с-удаленным-репозиторием) (
   git push origin master)
5. [Как синхронизироваться с удаленным репозиторием?](#Как-синхронизироваться-с-удаленным-репозиторием) (git pull origin
   master)
6. [Как сделать ветку?](#Как-сделать-ветку)
7. [Как переключиться между ветками?](#Как-переключиться-между-ветками)
8. [Как отменить локальные изменения?](#Как-отменить-локальные-изменения)
9. [Расскажи о команде git reset?](#Расскажи-о-команде-git-reset)
10. [Расскажите о команде git stash](#Расскажите-о-команде-git-stash)
11. [Расскажите о команде git cherry-pick](#Расскажите-о-команде-git-cherry-pick)
12. [Расскажите, о команде git rerere. Расскажите, как разрешаются конфликты.](#Расскажите-о-команде-git-rerere-Расскажите-как-разрешаются-конфликты)

## [Http протокол:](#Http-протокол)

1. [Расскажите, чем отличается UDP от TCP/IP.](#Расскажите-чем-отличается-UDP-от-TCP-IP)
2. Расскажите о методах HTTP протокола. Подробно рассказать отличие в заголовке и теле запросов: GET, POST, PUT,
   DELETE.[#Расскажите-о-методах-HTTP-протокола-Подробно-рассказать-отличие-в-заголовке-и-теле-запросов-GET-POST-PUT-DELETE]
3. [Расскажите о статусах HTTP протокола.](#Расскажите-о-статусах-HTTP-протокола)
4. [Расскажите о mime-type HTTP протокола.](Расскажите-о-mime-type-HTTP-протокола)

Веб-контейнер:

1. Что такое сервлет?
2. Какова структура веб-проекта?
3. Что такое контейнер сервлетов?
4. Каковы задачи, функциональность контейнера сервлетов?
5. Что вы знаете о сервлет фильтрах?
6. Зачем нужны слушатели в сервлетах?
7. Когда вы будете использовать фильтры, а когда слушатели?
8. Как обработать исключения, выброшенные другим сервлетом в приложении?
9. Что такое дескриптор развертывания?
10. Как реализовать запуск сервлета с запуском приложения?
11. Что представляет собой объект ServletConfig?
12. Что представляет собой объект ServletContext?
13. В чем отличия ServletContext и ServletConfig?
14. Что такое Request Dispatcher? Thymeleaf:

1. Расскажите о назначении Thymeleaf?
2. Расскажите про тег th:text.
3. Расскажите про тег th:if.
4. Расскажите про тег th:href.
5. Расскажите про тег th:each.
6. Расскажите про тег th:object.
7. Расскажите про тег th:action.

Spring boot:

1. Опишите из каких компонентов состоит Spring boot?
2. Что такое контроллер и как он связан с DispatcherServlet.
3. Что такое стереотипные аннотации? Какие они бывают?
4. Что такое DI? Как он реализован в Spring?
5. Опишите аннотации @ModelAttribute, @PathVariable, @RequestParam
6. Опишите аннотации @GetMapping, @PostMapping.

Многопоточность:

1. Объясните, где в веб приложении появляется многопоточность?
2. Как избежать проблем в многопоточности?
3. Как можно создать блокировку (deadlock) в сервлете?
4. Как решать проблемы многопоточности на уровне базы данных? Архитектура:

1. Объясните слоеную архитектуру.
2. Объясните шаблон MVC на примере Servlet.
3. Объясните шаблон MVC на примере Spring boot.

База данных.

1. Зачем нужен пул соединений? Безопасность.

1. [Что такое HttpSession?](#Что-такое-HttpSession)
2. [Что такое авторизация?](#Что-такое-авторизация)
3. [Что такое аутентификация?](#Что-такое-аутентификация)
4. [Что такое cookie?](#Что-такое-cookie)

## Git:

## 1 Что такое индекс?

[К оглавлению &#8593;](#Оглавление)

## 2 Как добавить изменения в индекс? (git add .)

[К оглавлению &#8593;](#Оглавление)

## 3 Как сделать фиксацию изменений? (git commit -m "text")

[К оглавлению &#8593;](#Оглавление)

## 4 Как сделать синхронизацию изменений с удаленным репозиторием? (git push origin master)

[К оглавлению &#8593;](#Оглавление)

## 5 Как синхронизироваться с удаленным репозиторием? (git pull origin master)

[К оглавлению &#8593;](#Оглавление)

## 6 Как сделать ветку?

[К оглавлению &#8593;](#Оглавление)

## 7 Как переключиться между ветками?

[К оглавлению &#8593;](#Оглавление)

## 8 Как отменить локальные изменения?

[К оглавлению &#8593;](#Оглавление)

## 9 Расскажи о команде git reset?

[К оглавлению &#8593;](#Оглавление)

## 10 Расскажите о команде git stash

[К оглавлению &#8593;](#Оглавление)

## 11 Расскажите о команде git cherry-pick

[К оглавлению &#8593;](#Оглавление)

## 12 Расскажите, о команде git rerere. Расскажите, как разрешаются конфликты.

[К оглавлению &#8593;](#Оглавление)

## Http протокол:

## 1 Расскажите, чем отличается UDP от TCP/IP.

[К оглавлению &#8593;](#Оглавление)

## 2 Расскажите о методах HTTP протокола. Подробно рассказать отличие в заголовке и теле запросов: GET, POST, PUT, DELETE.

[К оглавлению &#8593;](#Оглавление)

## 3 Расскажите о статусах HTTP протокола.

[К оглавлению &#8593;](#Оглавление)

## 4 Расскажите о mime-type HTTP протокола.

[К оглавлению &#8593;](#Оглавление)

## Веб-контейнер:

## 1 Что такое сервлет?

[К оглавлению &#8593;](#Оглавление)

## 2 Какова структура веб-проекта?

[К оглавлению &#8593;](#Оглавление)

## 3 Что такое контейнер сервлетов?

[К оглавлению &#8593;](#Оглавление)

## 4 Каковы задачи, функциональность контейнера сервлетов?

[К оглавлению &#8593;](#Оглавление)

## 5 Что вы знаете о сервлет фильтрах?

[К оглавлению &#8593;](#Оглавление)

## 6 Зачем нужны слушатели в сервлетах?

[К оглавлению &#8593;](#Оглавление)

## 7 Когда вы будете использовать фильтры, а когда слушатели?

[К оглавлению &#8593;](#Оглавление)

## 8 Как обработать исключения, выброшенные другим сервлетом в приложении?

[К оглавлению &#8593;](#Оглавление)

## 9 Что такое дескриптор развертывания?

[К оглавлению &#8593;](#Оглавление)

## 10 Как реализовать запуск сервлета с запуском приложения?

[К оглавлению &#8593;](#Оглавление)

## 11 Что представляет собой объект ServletConfig?

[К оглавлению &#8593;](#Оглавление)

## 12 Что представляет собой объект ServletContext?

[К оглавлению &#8593;](#Оглавление)

## 13 В чем отличия ServletContext и ServletConfig?

[К оглавлению &#8593;](#Оглавление)

## 14 Что такое Request Dispatcher? Thymeleaf:

[К оглавлению &#8593;](#Оглавление)

## 1 Расскажите о назначении Thymeleaf?

[К оглавлению &#8593;](#Оглавление)

## 2 Расскажите про тег th:text.

[К оглавлению &#8593;](#Оглавление)

## 3 Расскажите про тег th:if.

[К оглавлению &#8593;](#Оглавление)

## 4 Расскажите про тег th:href.

[К оглавлению &#8593;](#Оглавление)

## 5 Расскажите про тег th:each.

[К оглавлению &#8593;](#Оглавление)

## 6 Расскажите про тег th:object.

[К оглавлению &#8593;](#Оглавление)

## 7 Расскажите про тег th:action.

[К оглавлению &#8593;](#Оглавление)

## Spring boot:

## 1 Опишите из каких компонентов состоит Spring boot?

[К оглавлению &#8593;](#Оглавление)

## 2 Что такое контроллер и как он связан с DispatcherServlet.

[К оглавлению &#8593;](#Оглавление)

## 3 Что такое стереотипные аннотации? Какие они бывают?

[К оглавлению &#8593;](#Оглавление)

## 4 Что такое DI? Как он реализован в Spring?

[К оглавлению &#8593;](#Оглавление)

## 5 Опишите аннотации @ModelAttribute, @PathVariable, @RequestParam

[К оглавлению &#8593;](#Оглавление)

## 6 Опишите аннотации @GetMapping, @PostMapping.

[К оглавлению &#8593;](#Оглавление)

## Многопоточность:

## 1 Объясните, где в веб приложении появляется многопоточность?

[К оглавлению &#8593;](#Оглавление)

## 2 Как избежать проблем в многопоточности?

[К оглавлению &#8593;](#Оглавление)

## 3 Как можно создать блокировку (deadlock) в сервлете?

[К оглавлению &#8593;](#Оглавление)

## 4 Как решать проблемы многопоточности на уровне базы данных? Архитектура:

[К оглавлению &#8593;](#Оглавление)

##           

## 1 Объясните слоеную архитектуру.

[К оглавлению &#8593;](#Оглавление)

## 2 Объясните шаблон MVC на примере Servlet.

[К оглавлению &#8593;](#Оглавление)

## 3 Объясните шаблон MVC на примере Spring boot.

[К оглавлению &#8593;](#Оглавление)

## База данных.

[К оглавлению &#8593;](#Оглавление)

## 1 Зачем нужен пул соединений?

[К оглавлению &#8593;](#Оглавление)

## Безопасность.

[К оглавлению &#8593;](#Оглавление)

## 1 Что такое HttpSession?

[К оглавлению &#8593;](#Оглавление)

## 2 Что такое авторизация?

[К оглавлению &#8593;](#Оглавление)

## 3 Что такое аутентификация?

[К оглавлению &#8593;](#Оглавление)

## 4 Что такое cookie?

[К оглавлению &#8593;](#Оглавление)